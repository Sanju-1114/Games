class Dice {
  ...
  roll() {
    ...
    // After animation, show the result and run game logic
    setTimeout(() => {
      // End animation
      document.getElementById("die").classList.remove("rolling");

      // All your original logic goes here, inside this setTimeout
      if (this.type == 1) {
        msg.innerHTML = "Red";
        msg.style.color = "Red";
      }
    ...
    }, 1000); // Matches typical CSS animation duration
  }
}
class Red_g {
  j = 0;
...
class Red {
  ...
  checker() {
    if (
      ...
      R4.home &&
      roll.count != 6 &&
      roll.rctns[roll.rctns.length - 1] != 6
    ) {
      return false;
    } else {
      if (roll.count == 6) {
...
class Blue {
  ...
  checker() {
    if (
      ...
      B4.home &&
      roll.count != 6 &&
      roll.bctns[roll.bctns.length - 1] != 6
    ) {
      return false;
    } else {
      if (roll.count == 6) {
...
class Green {
  ...
  checker() {
    if (
      ...
      G4.home &&
      roll.count != 6 &&
      roll.gctns[roll.gctns.length - 1] != 6
    ) {
      return false;
    } else {
      if (roll.count == 6) {
...
class Yellow {
  ...
  checker() {
    if (
      ...
      Y4.home &&
      roll.count != 6 &&
      roll.yctns[roll.yctns.length - 1] != 6
    ) {
      return false;
    } else {
      if (roll.count == 6) {
